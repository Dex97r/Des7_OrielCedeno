<?php
  ...
  class UpdatePicture{
    ...
    static function UpdatePic(){
        ...
        $id_empresa = UtilitiesMethod::existCompany($_SESSION['id']);
        ...
        // Verificar que el archivo sea una imagen
            if (strpos($tipo_foto, 'image') !== false) {
                ...
                $sql = "UPDATE empresas SET logo = :foto WHERE id = :id";
    ...
    static function Delete(){
        ...
        $estado = 'undefined';
        $id_usuario = UtilitiesMethod::existCompany($_SESSION['id']);
        $sql = "UPDATE empresas SET logo = :estado WHERE id = :id";
        $conexion = Conexion::getConexion();
        $stmt = $conexion->prepare($sql);
       ...
       try {
            if ($stmt->rowCount() > 0) {
                if(UtilitiesMethod::borrarFoto($id_usuario)){
                    $array['error'] = false;
                    $array['alert'] = 'empresa.php';
                }                
            }
